...
const createUserSchema = z.object({
  ...
  role: z.enum(['owner', 'admin', 'staff', 'doctor', 'patient']).default('staff'),
});
...
export const GET = withSuperAdminAuth(async (request: NextRequest) => {
  try {
    ...
    return NextResponse.json(
      { error: getErrorMessage(error, 'Failed to fetch tenant users') },
      { status: 500 }
    );
  }
});
...
export const POST = withSuperAdminAuth(async (request: NextRequest) => {
  try {
    ...
    console.error('POST tenant user error:', error);
...
export const PATCH = withSuperAdminAuth(async (request: NextRequest) => {
  try {
    ...
    const body = await request.json();
    const { userId, name, email, role } = updateUserSchema.parse(body);
    ...
    console.error('PATCH tenant user role error:', error);
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: 'Validation error', details: error.errors },
        { status: 400 }
